 ...
 * 
 */
public class PluginContainerActivity extends BaseActivity {
	private SiteSpec site;
	private FileSpec file;
	private String fragmentName;
	private boolean loaded;
	private WXClassLoader classLoader;
	private WXResources myResources;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		do {
			...
			classLoader = WXClassLoader.getClassLoader(site, file);
		...
		if (!loaded) {
			...
			text.setText("Unable to load plugin " + (error == 0 ? "" : " #" + error));
		...
		try {
			...
			error = 211; // #211
			TextView text = new TextView(this);
			text.setText("Unable to load plugin #" + error);
			text.append("\n" + e);
			text.setLayoutParams(new FrameLayout.LayoutParams(
					ViewGroup.LayoutParams.WRAP_CONTENT,
					ViewGroup.LayoutParams.WRAP_CONTENT, Gravity.CENTER));
	...
	public Intent urlMap(Intent intent) {
		do {
			...
			if (uri.getScheme() == null)
				break;
			if (!(PluginManager.PRIMARY_SCHEME.equalsIgnoreCase(uri.getScheme())))
				break;
	...
	public WXResources getOverrideResources() {
		return myResources;
	}

	void setOverrideResources(WXResources myres) {
		if (myres == null) {
